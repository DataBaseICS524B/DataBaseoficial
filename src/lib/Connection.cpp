...
public:
    ConnectionImpl(const std::string& host, int port) : host_(host), port_(port), sock_(-1) {}
    ...
    std::string execute(const std::string& query) {
        ...
        uint32_t queryLen = htonl(query.length());
        send(sock_, reinterpret_cast<char*>(&queryLen), 4, 0);
        send(sock_, query.c_str(), query.length(), 0);
        
        uint32_t status, dataLen;
...
bool Connection::connect() { return pImpl_->connect(); }
void Connection::disconnect() { pImpl_->disconnect(); }
bool Connection::isConnected() const { return pImpl_->isConnected(); }
std::string Connection::execute(const std::string& query) { return pImpl_->execute(query); }
